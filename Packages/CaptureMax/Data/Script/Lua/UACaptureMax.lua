...
UTClass.UACaptureMax(UTActivity)

-- state dependencies

UACaptureMax.State = {}

	require "UACaptureMax.State.RoundLoop"

-- default

UACaptureMax.bitmap = "base:texture/ui/loading_CaptureMax.tga"

-- __ctor --------------------------------------------------------------------

function UACaptureMax:__ctor(...)
    ...
    self.settings = {
        ...
        [1] = { title = l"titlemen006", options = {
            ...
            [1] = { displayMode = nil, label = l"goption008", tip = l"tip160", choices = { { value = 2 }, { value = 3, conditional = true }, { value = 4, conditional = true } }, index = "numberOfTeams", condition = function (self) return (game.settings.addons.medkitPack == 1) end },
            ...
            [11] = { displayMode = nil, label = l"goption063", tip = l"tip253", choices = { { value = 0, displayMode = "large", text = l"but029" }, { value = 1, displayMode = "large", text = l"but030", conditional = true } }, index = "flagmode", condition = function (self) return (game.settings.addons.customPack == 1) end },
    ...
    self.states["roundloop"] = UACaptureMax.State.RoundLoop:New(self)
...
function UACaptureMax:__dtor()
end

-- InitEntityBakedData  ------------------------------------------------------

function UACaptureMax:InitEntityBakedData(entity, ranking)
...
function UACaptureMax:InitEntityHeapData(entity, ranking)
	...
	if (entity:IsKindOf(UTTeam)) then
		...
		if (entity.primary) then
			entity.gameplayData = { 0x00, 0x06, 0x00, 0x10, 0x00, 0x10, 0x00, 0x10 }
		else
			entity.gameplayData = { 0x00, 0x00, 0x00, 0x10, 0x00, 0x10, 0x00, 0x10 }
...
function UACaptureMax:UpdateEntityBakedData(entity, ranking)
